...
class ConsolidatedStoresAssetsData  implements FromCollection,WithHeadings, ShouldAutoSize, WithEvents
{
    ...
    */
    ...
    { 
       ...
       ->join('stores_suppliers as t2','t1.supplier_id','t2.id')
       ->leftjoin('stores_asset_depreciation as t4','t4.asset_id','t1.id')
       ->leftjoin('stores_depreciation_methods as t6','t6.id','depreciation_method_id')
    //    ->leftjoin('ar_asset_checkout_details as t3','t3.asset_id','t1.id')
    //    ->leftjoin('users as t4','t4.id','t3.id')
      ->join('stores_statuses as t3','t3.id','t1.status_id')
     ->leftjoin('stores_sites as t5','t5.id','t1.site_id')
       ...
       foreach($results as $result)
       {
        ...
        //CONCAT_WS(' ',decrypt(t4.first_name),decrypt(t4.middlename),decrypt(t4.last_name)) as assigned_user
         $checkout_details = db::table('stores_asset_checkout_details as t3')
         ->leftjoin('users as t4','t4.id','t3.user_id')
         ->join('stores_sites as t5','t5.id','t3.checkout_site_id')
